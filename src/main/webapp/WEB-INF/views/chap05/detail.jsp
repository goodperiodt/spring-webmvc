...
<body>
    ...
    <script>
        ...
        // 서버에 실시간으로 비동기통신을 해서 JSON을 받아오는 함수를 선언
        function fetchGetReplies() {
            // fetch 함수를 통해 비동기 통신 진행할 때 GET요청은 요청에 관련한 객체를 따로 요청하지 않는다.
            // method를 get이라고 요청하지 않고, 데이터 전달 시에는 URL에 포함시켜서 전달한다.
            // ***자바스크립트 문자열 안에 달러와 중괄호를 쓰면 el로 인식, 주석에도 달러와 중괄호를 쓰면 인식한다.***
            // 만약 템플릿 리터럴 문자를 쓰고 싶다면 앞에 백슬러시\문자를 붙여줘야 한다.
            // fetch(URL+'/'+bno)
            fetch(`\${URL}/\${bno}`)
                .then(res => res.json())
                .then(replyList => {
        ...
        $addBtn.onclick = e => {
            ...
            const requestInfo = {
                ...
                body: JSON.stringify(payload) // 바디에 전송하고자 하는 데이터를 JSON으로 변환해서 전송
                // js객체를 JSON으로 변환해서 body에 추가
            }
            ...
            fetch(URL, requestInfo)
                ...
                .then(res => {
                    ...
                    if (res.status === 200) {
                        ...
                        alert('입력 값에 문제가 있습니다. 입력값을 다시 확인해보세요');
                        return res.text();
                    }
                })
                .then(data => {
                    console.log('응답 성공! ', data);
                    // 댓글 작성자 input과 댓글 내용 text를 지워준다.
                    $replyText.value = '';
                    $replyWriter.value = '';
